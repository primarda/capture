...
packets = PacketFu::Capture.new(:iface=>dev, :start=>true, :filter=>FILTER)
...
count = 0
packets.stream.each do |pkt|
  count = count + 1
  #puts Time.now.strftime("%F %T.%6N")
  if PacketFu::TCPPacket.can_parse?(pkt)
    tcp_packet = PacketFu::TCPPacket.parse(pkt)
    ...
    if body.match(/Req_\d*/) or body.match(/Seq_\d*/)
      ...
      print Time.now.strftime("%F %T.%6N")
      print","
